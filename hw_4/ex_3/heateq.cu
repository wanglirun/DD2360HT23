...
int main(int argc, char **argv) {
  ...
  cusparseSpMatDescr_t SpAdescriptor;   // Sparse Mat descriptor needed by cuSPARSE
  cusparseDnVecDescr_t DnTempdescriptor;   // Dense Vec descriptor needed by cuSPARSE
  cusparseDnVecDescr_t DnTmpdescriptor;   // Dense Vec descriptor needed by cuSPARSE
  ...
  gpuCheck(cudaMallocManaged(&ARowPtr, dimX * sizeof(int)));
  ...
  //@@ Insert code to set the cuBLAS pointer mode to CUBLASE_POINTER_MODE_HOST
  ...
  cusparseCheck(cusparseCreateMatDescr(&Adescriptor)); // default values are correct
  cusparseCheck(cusparseCreateCsr(&SpAdescriptor, dimX, dimX, nzv,
                    ARowPtr, AColIndx, A,
                    CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO,
                    CUDA_R_64F));
  cusparseCheck(cusparseCreateDnVec(&DnTempdescriptor, dimX, temp, CUDA_R_64F));
  cusparseCheck(cusparseCreateDnVec(&DnTmpdescriptor, dimX, tmp, CUDA_R_64F));
  ...
  cputimer_stop("Performing relaxation");
